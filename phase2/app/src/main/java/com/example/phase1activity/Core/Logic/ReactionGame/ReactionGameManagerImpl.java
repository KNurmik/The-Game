...
public class ReactionGameManagerImpl implements ReactionGameManager {
  /** The user's score. */
  private int score = 0;
  ...
  /**
   * Initialize ReactionGameManagerImpl. Tell Dagger how to construct this object.
   *
   * @param difficulty represents the difficulty desired. Potential Phase 2 extension adds more
   *     difficulty settings.
   */
  @Inject
  public ReactionGameManagerImpl(String difficulty) {
  ...
  /** @return the fastest reaction in milliseconds for the user. */
  public int getFastestReaction() {
    return (int) Math.floor(fastestReaction * 100) / 100;
  }
